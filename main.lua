...
gameWidth, gameHeight = 350, 288 -- allows for a constant
...
function love.load() -- Load in data when the application starts up | we put things in here we want to load | Only runs once
  ...
  building3 = botBuildings.BotBuilding:new(gameHeight, buildingPosChange, gameWidth + 200) -- starts off screen to make sure that they're staggered
  building4 = topBuildings.TopBuilding:new(gameHeight, buildingPosChange, gameWidth + 200) -- as they're paired, they must be at the same position
...
function love.draw() -- Draws to the screen | We first move the player's location, then we visually draw this on the screen | Player is hit, heart destroyed or damaged
  ...
  if game.state.paused == true then -- call the pause
    drawPause() 
  end
  if game.state.ended == true then
    love.graphics.print("Final score: " .. score, (gameWidth / 2) -50, (gameHeight / 2) - 50) -- displays it middle of screen with x adjustng when needed
    love.graphics.print("Press SPACE to restart!", ((gameWidth / 2) -75), (gameHeight / 2) - 75)
  end  
  push:finish() -- anything drawn via push has to be ended here
end
...
function drawPause() -- pause screen, will stop all possible updates and exist outside the main game loop
  ...
  if game.state.ended == false then
...
function love.keypressed(key) -- "Opens the pause" | Currently pressing escape will stop the game, then pressing escape again will begin it.
  if key == "escape" then
