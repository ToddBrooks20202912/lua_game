...
function love.update(dt) -- Runs every 60 frames | "dt" is deltatime, time between the frame and the last, every frame (every frame is a second) | updates everything (player moving)
  ...
  if (game.state.menu == false and game.state.paused == false) then -- this is at the moment, just a basic test
    num = num + 1
    text = num
  end
...
function love.draw() -- Draws to the screen | We first move the player's location, then we visually draw this on the screen | Player is hit, heart destroyed or damaged
  push:start()
  if not game.state.paused then -- all main game logic goes here, otherwise it will move onto the other conditionals to check for the state of the game 
...
function drawMenu()
  love.graphics.rectangle("fill", (gameWidth / 2) - 50, (gameHeight / 2) - 50, 100, 100)
end
